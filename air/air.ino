...
const int PIN_FC_RX = 16;
const int PIN_FC_TX = 17;

// ╔══════════════════════════════════════════════════════════╗
// ║  📡 GROUND UNIT MAC ADDRESS — এখানে বসাও!                ║
// ╠══════════════════════════════════════════════════════════╣
// ║                                                          ║
// ║  STEPS:                                                  ║
// ║  1. MAC Finder sketch GROUND ESP32 এ upload করো         ║
// ║  2. Serial Monitor (115200) এ MAC দেখো                  ║
// ║  3. সেই MAC নিচের array তে paste করো                    ║
// ║                                                          ║
// ║  উদাহরণ: যদি Ground MAC হয় F0:24:F9:0E:0C:F0           ║
// ║  তাহলে নিচে এভাবে লিখবে:                                ║
// ║  {0xF0, 0x24, 0xF9, 0x0E, 0x0C, 0xF0}                   ║
// ║                                                          ║
// ╚══════════════════════════════════════════════════════════╝
uint8_t groundMac[] = {0xF0, 0x24, 0xF9, 0x0E, 0x0C, 0xF0};
//                       👆 এই 6 টা value GROUND এর MAC দিয়ে replace করো

// ============================================================
// SETTINGS
// ============================================================
#define FC_BAUD         57600     // ArduPilot SERIAL1_BAUD = 57

// LoRa RF (Ground এর সাথে EXACT match হতে হবে)
...
#define GND_SLOT_MS     23

// ============================================================
// 💡 LED LOGIC
//    LoRa mode    → LED ON  (long range, slow)
//    ESP-NOW mode → LED OFF (close range, fast)
// ============================================================
...
uint8_t  fcBuf[FC_BUF_SIZE];
volatile int fcHead = 0, fcTail = 0;

static inline int  fcAvail() { return (fcHead - fcTail + FC_BUF_SIZE) % FC_BUF_SIZE; }
static inline void fcPush(uint8_t b) {
  int n = (fcHead + 1) % FC_BUF_SIZE;
  if (n != fcTail) { fcBuf[fcHead] = b; fcHead = n; }
}
static inline uint8_t fcPop() {
  uint8_t b = fcBuf[fcTail]; fcTail = (fcTail + 1) % FC_BUF_SIZE; return b;
}
static inline void fcClear() { fcHead = fcTail = 0; }
static inline void drainFC() { while (Serial2.available()) fcPush(Serial2.read()); }
...
enum LRState { LR_TX, LR_RX_WAIT };
LRState       lrState = LR_TX;
unsigned long lrTs    = 0;
...
void loraStart() {
  ...
  LoRa.enableCrc();
  LoRa.receive();
  lrState = LR_TX; lrTs = millis();
}
void loraStop() { LoRa.sleep(); LoRa.end(); }

void loraTx() {
  drainFC();
  int n = fcAvail(); if (n > MAX_PAYLOAD) n = MAX_PAYLOAD;

  txFrame[0] = SYNC_BYTE;
  txFrame[1] = (n > 0) ? FT_DATA : FT_KEEPALIVE;
  txFrame[2] = (uint8_t)n;
  for (int i = 0; i < n; i++) txFrame[3 + i] = fcPop();
...
bool loraRx() {
  ...
  if (hdr[1] == FT_DATA && len > 0) Serial2.write(pl, len);
  return true;
}
...
void espStart() {
  ...
  // 📡 Ground unit কে peer হিসেবে add করছি
  esp_now_peer_info_t p = {};
  memcpy(p.peer_addr, groundMac, 6);    // ← এখানে groundMac use হচ্ছে
...
void enterLora() {
  ...
  fcClear();
  loraStart();
  digitalWrite(PIN_LED, LED_ON_FOR_LORA);     // 💡 LED ON
...
void enterEspnow() {
  ...
  fcClear();
  espStart();
  digitalWrite(PIN_LED, LED_ON_FOR_ESPNOW);   // 💡 LED OFF
...
void setup() {
  ...
  Serial2.begin(FC_BAUD, SERIAL_8N1, PIN_FC_RX, PIN_FC_TX);
...
void loop() {
  ...
  if (currentMode == MODE_ESPNOW) {
    // 1. flush received bytes to FC
    if (espRxLen > 0) {
      noInterrupts();
      int len = espRxLen;
      espRxLen = 0;
      interrupts();
      Serial2.write(espRxBuf, len);
    }
    ...
    if (avail > 0) {
      ...
      esp_now_send(groundMac, buf, avail);   // ← groundMac এ পাঠাচ্ছে
    }
    return;
  }
  ...
  switch (lrState) {
    case LR_TX:
      ...
      break;
    case LR_RX_WAIT:
      if (loraRx())                       lrState = LR_TX;
      else if (now - lrTs >= GND_SLOT_MS) lrState = LR_TX;
      break;
  }
}
